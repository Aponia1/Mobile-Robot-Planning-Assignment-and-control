                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  zw(1)=0;
  zw(2)=1;
  v_dot=-g_*zw+(R*(zw*thrust)+external_force_)*1/mass_;
  acc_ = v_dot;

  R_dot = R * omega_vee;
  //请在这里补充完四旋翼飞机的动力学模型，角速度导数的计算涉及到惯性矩阵J_的逆、力矩、科里奥利力（通过角速度与惯性矩阵和角速度的叉积来计算）和外部力矩等因素。
  // omega_dot = //??????
  omega_dot=J_.inverse()*(moments+external_moment_)-J_.inverse()*omega_vee*J_*cur_state.omega;
  //ROS_INFO("v_dot=(%.2f,%.2f,%.2f)  omega_dot=(%.2f,%.2f,%.2f)\n",v_dot(0),v_dot(1),v_dot(2),omega_dot(0),omega_dot(1),omega_dot(2));
  //zw(2)=0;
  //omega_dot =zw;
