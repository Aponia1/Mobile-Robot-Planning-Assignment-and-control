...
bool Astarpath::AstarSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  while (!Openset.empty()) {
    ...
    if(currentPtr->coord==endPtr->coord){
          //ROS_INFO("findway\n");
          //ROS_INFO("from(%f,%f,%f)to",startPtr->coord(0),startPtr->coord(1),startPtr->coord(2));
          //ROS_INFO("(%f,%f,%f)\n",currentPtr->coord(0),currentPtr->coord(1),currentPtr->coord(2));
          //ROS_INFO("(%f,%f,%f)\n",endPtr->coord(0),endPtr->coord(1),endPtr->coord(2));
    ...
    for(unsigned int i=0;i<neighborPtrSets.size();i++)
    {
      ...
      if(neighborPtr->id==0)///////////////////////////////////change
      {
        ...
        //ROS_INFO("(%f,%f,%f)\n",neighborPtr->coord(0),neighborPtr->coord(1),neighborPtr->coord(2));
        continue;
      }
